...
class pairWiseModel(nn.Module):
    ...
    def sped_up_loss(self, target):
        ...
        ##get pairs of docs for query
        lambda_ji = 0
        for p_1 in tqdm(range(self.pairs), position=0):
            lambda_i = 0
            # pick a doc at random
            i = np.random.choice(self.scores.shape[0])
            # score for document i
            s_i = self.scores[i]
            for p_2 in range(self.pairs):
                # pick a doc at random
                j = np.random.choice(self.scores.shape[0])
                ...
                if target[i] > target[j]:
                    S = 1
                    lambda_ij = (self.calc_ranknet(s_i, s_j))
                    lambda_i += lambda_ij
                elif target[i] == target[j]:
                    S = 0
                else:
                    S = -1
                    lambda_ij = (self.sigma * (0.5 * (1 - S) - self.calc_ranknet(s_i, s_j)))
                    lambda_i -= lambda_ij
            S = torch.Tensor([S])
            loss += self.loss_fn(lambda_i, S)
