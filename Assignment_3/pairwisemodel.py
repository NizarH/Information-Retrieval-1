...
def trainModel(model, data, epochs, optimizer):
    ...
    labels = torch.Tensor(data.train.label_vector).to("cpu").unsqueeze(1)
    labels_val = torch.Tensor(data.validation.label_vector).to("cpu").unsqueeze(1)
